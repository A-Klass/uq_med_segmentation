...
class SWAG(torch.nn.Module):
    ...
    def sample(self, scale=1.0, cov=False, seed=None, block=False, fullrank=True, cuda=True):
        if seed is not None:
            torch.manual_seed(seed)
        if not block:
            self.sample_fullrank(scale, cov, fullrank, cuda=cuda)
        else:
    ...
    def sample_fullrank(self, scale, cov, fullrank, cuda=True):
